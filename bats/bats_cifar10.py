...
def cola_loss_adjustment(model_output, labels, original_loss, cola_factor):
    ...
    Args:
        model_output: 模型对对抗样本的输出 [batch_size, num_classes]
        labels: 真实标签 [batch_size]
        original_loss: 原始损失值 [batch_size]
        cola_factor: 损失适应因子 η_c (0,1)

    Returns:
        adjusted_loss: 调整后的损失
    """
    with torch.no_grad():
        # 获取预测结果
    ...
    return adjusted_loss.mean()  # 返回平均损失
...
def main():
    ...
    criterion = nn.CrossEntropyLoss(reduction='none')  # 修改为返回每个样本的损失
    ...
    for epoch in range(args.epochs):
        ...
        # save checkpoints
        ckpt_name = args.model + "_CIFAR10_TDAT_COLA_robustAcc_" + str(pgd_acc) + "_clean_acc_" + str(test_acc) + ".pt"
